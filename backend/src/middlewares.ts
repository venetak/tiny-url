...
export function validateUrlInput(
  ...
  try {
    ...
    if (/^(javascript:|data:)/i.test(longUrl)) {
      return res
        .status(400)
        .json({ status: 'error', error: 'Malicious URL scheme detected.' });
    }

    // Disallow script tags or suspicious characters
    if (
      /<script|onerror=|onload=|<img|<iframe|<svg|<object|<embed|<link|<style|<base|<form|<input|<textarea|<button|<select|<option|<a\s+href=/i.test(
        longUrl,
      )
    ) {
      return res
        .status(400)
        .json({ status: 'error', error: 'Malicious content detected in URL.' });
    }

    // Disallow SQL injection patterns (very basic)
    if (
      /('|"|;|--|\b(SELECT|INSERT|UPDATE|DELETE|DROP|UNION|OR|AND)\b)/i.test(
        longUrl,
      )
    ) {
      return res
        ...
        .json({
          status: 'error',
          error: 'Suspicious characters detected in URL.',
        });
    }
    next();
  } catch (err) {
